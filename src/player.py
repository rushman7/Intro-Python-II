...
class Player:
  ...
  def search(self):
    if self.current_room.is_light == True or all_items['lamp'] in self.inventory or all_items['lamp'] in self.current_room.items:
      ...
      print("\033[0;31;49mIt's too dark in here. Maybe you need a lamp..?")
  
  def add_to_inventory(self):
    if self.current_room.is_light == True or all_items['lamp'] in self.inventory or all_items['lamp'] in self.current_room.items:
      if len(self.current_room.items) <= 0:
        print('\033[0;37;49mThere is no item to add')
      else:
